...
public class UpdateStatement {
	...
	public static void main(String[] args) {
		...
		try {
			...
			int record= stmt.executeUpdate("UPDATE Employees SET first='sigrid' WHERE id=108");
			//if record = 0  means no record inserted if record >0 means record inserted successful
			if (record>0) {
				
				System.out.println("Congratulation recors Update success");
			} else {
				System.err.println("Error coccured while Updating the data");
			}
